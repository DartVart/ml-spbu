...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_head, with_synthesizer):
        # n_embd: embedding dimension, n_head: the number of heads we'd like
        super().__init__()
        head_size = n_embd // n_head
        self.sa = MultiHeadAttention(n_head, head_size, with_synthesizer=with_synthesizer)
...
class GPTLanguageModel(nn.Module):
    def __init__(self, with_synthesizer: bool = True, device='cpu'):
        super().__init__()
        # each token directly reads off the logits for the next token from a lookup table
        self.token_embedding_table = nn.Embedding(vocab_size, n_embd)
        self.position_embedding_table = nn.Embedding(block_size, n_embd)
        self.blocks = nn.Sequential(
            *[Block(n_embd, n_head=n_head, with_synthesizer=with_synthesizer) for _ in range(n_layer)])
...
if __name__ == '__main__':
    ...
    model = GPTLanguageModel(True, device)
    m = model.to(device)
    ...
    for iter in range(max_iters):
        ...
        if iter % eval_interval == 0 or iter == max_iters - 1:
            ...
            for split in ['train', 'val']:
                all_losses[split].append(float(losses[split]))
            all_losses['iter'].append(iter)

            torch.save(m.state_dict(), f'new_model/{iter}_iter.pth')
    ...
    losses_df = pd.DataFrame(data=all_losses)
    losses_df.to_csv('new_model/losses.csv')

    # draw losses
    train_perplexities = np.exp2(losses_df['train'])
    test_perplexities = np.exp2(losses_df['val'])
    iterations = losses_df['iter']

    plt.plot(iterations, train_perplexities, label='train')
    plt.plot(iterations, test_perplexities, label='test')
    plt.title('Losses')
    plt.xlabel('training iteration')
    plt.ylabel('Perplexity')
    plt.savefig('new_model/losses.png')
